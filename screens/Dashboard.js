...
const BASE_SIZE = nowTheme.SIZES.BASE;
const GRADIENT_BLUE = ['#6B84CA', '#8F44CE'];
const GRADIENT_PINK = ['#D442F8', '#B645F5', '#9B40F8'];
const COLOR_WHITE = nowTheme.COLORS.WHITE;
const COLOR_GREY = nowTheme.COLORS.MUTED; // '#D8DDE1';
...
class Dashboard extends React.Component {
  renderHeader = () => (
    <NavBar
      ...
      leftIconColor={nowTheme.COLORS.MUTED}
      right={(
        <Button
          color="transparent"
        ...
        >
          <Icon size={BASE_SIZE} name="heart" family="font-awesome" color={nowTheme.COLORS.MUTED} />
        </Button>
      )}
      style={Platform.OS === 'android' ? { marginTop: nowTheme.SIZES.BASE } : null}
    />
  )
    ...
    const GradientStats = () => (
      <Defs key="gradient">
        <LinearGradient id="gradient" x1="0" y="0%" x2="0%" y2="100%">
          <Stop offset="0%" stopColor={nowTheme.COLORS.THEME} />
          <Stop offset="100%" stopColor={nowTheme.COLORS.INFO} />
        </LinearGradient>
      </Defs>
    );
    ...
    return (
      <Block style={{ marginBottom: BASE_SIZE * 3 }}>
        ...
        <Block row space="evenly" style={{ marginTop: BASE_SIZE }}>
          {statsTitles.map(title => <Text key={title} size={nowTheme.SIZES.FONT * 0.85} muted>{title}</Text>)}
        </Block>
      </Block>
    );
  }
